...
async function main() {
  ...
  const mint = await MinerMint.deploy()
  await mint.deployed()

  console.log("MinerMint implement address is:", mint.address)

  let miner_mint_address = mint.address;

  const Mint = await hre.ethers.getContractAt('MinerMint', miner_mint_address, signer)
  const initialize_data = await Mint.populateTransaction.initialize(
    my_address,
    miner_address, 
    repo_address,
    busd_address
  ...
  let proxy = await BTCZProxy.deploy(miner_mint_address, initialize_data.data)
